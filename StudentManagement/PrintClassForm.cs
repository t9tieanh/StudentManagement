...
{
    public partial class PrintClassForm : Form
    {
        public PrintClassForm()
        {
            InitializeComponent();
            GetMyClass();
        }
        public PrintClassForm(Teacher_DTO myTeacher)
        {
            InitializeComponent();
            GetMyClass(myTeacher);
        }
        ...
        #region hàm get Class
        private void SetMyClass (DataTable dt)
        {
            try
            {
                ...
                DataGridViewImageColumn pic = (DataGridViewImageColumn)dgvClass.Columns[5];
            ...
            dgvClass.Columns.Insert(0, column);
            // Xử lý sự kiện khi số lượng hàng thay đổi
            dgvClass.RowsAdded += DataGridView1_RowsAdded;
            dgvClass.RowsRemoved += DataGridView1_RowsRemoved;
        }
        private void GetMyClass (Teacher_DTO myTeacher) 
        {

            try
            {
                DataTable dt = BUL.Class_BUL.GetDataClass (myTeacher);
                lblTitleClass.Text += " của giáo viên " + myTeacher.ToString();
                SetMyClass(dt);
            }
            catch
            {
        ...
        private void GetMyClass()
        {
            try
            {
                DataTable dt = BUL.Class_BUL.GetDataClass();
                SetMyClass(dt);
            }
            catch
            {
        ...
        private void DataGridView1_RowsAdded(object sender, DataGridViewRowsAddedEventArgs e)
        {
            ...
            for (int i = e.RowIndex; i < dgvClass.Rows.Count; i++)
            {
                dgvClass.Rows[i].Cells[0].Value = (i + 1).ToString();
            }
        }
